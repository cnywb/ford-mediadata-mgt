        ...
        controller: ['$scope', 'dbImService', function ($scope, dbImService) {
            ...
            angular.forEach($scope.dbForm.sections, function (section) {
                angular.forEach(section.fields, function (field) {
                    ...
                    if (!angular.isUndefined($scope.dbForm.events)) {
                        ...
                        var selectEvent = $scope.dbForm.events[field.name + 'Select'];
                        if (selectEvent) {
                            field.select = selectEvent;
                        }
                    }
            ...
            function resetData(data) {
                angular.forEach(selectFields, function (field) {
                    if (data[field.name] && (!angular.isObject(data[field.name]) || angular.isArray(data[field.name]))) {
                        ...
                        } else if (data[field.name] && angular.isArray(field.dropDownItem) && field.dropDownItem != "") {
                            var dictObjArray = [];
