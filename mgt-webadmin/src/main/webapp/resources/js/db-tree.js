    ...
    return {
        ...
        controller: ['$scope', function ($scope) {
            ...
            $scope.dbTree.itemClick = function (item, parent) {
                ...
                angular.forEach($scope.dbTreeData,function (item,index) {
                    item.checked=false;
                    angular.forEach(item.children,function (i,index) {
                        i.checked=false;
                        angular.forEach(i.children,function (x,index) {
                            x.checked=false;
                            if(x.children.length>0){
                                angular.forEach(x.children,function (y,j) {
                                    y.checked=false;
                                    if(y.children.length>0){
                                        angular.forEach(y.children,function (z,l) {
                                            z.checked=false;
                                        })
                                    }
                                });
            ...
            $scope.dbTree.setData = function (data) {
                ...
                angular.forEach(data,function (item,index) {
                    item.checked=false;
                    angular.forEach(item.children,function (i,n) {
                        i.checked=false;
                        angular.forEach(i.children,function (x,m) {
                            x.checked=false;
                            if(x.children.length>0){
                                angular.forEach(x.children,function (y,j) {
                                    y.checked=false;
